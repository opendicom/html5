...
from radiologo import models, serializers
import json
import requests
import urllib
...
def index(request):
    return render_to_response('radiologo/index.html')


def report_editor(request, *args, **kwargs):
    context = {'study_iuid': kwargs['study_iuid']}
    return render(request, 'radiologo/editor.html', context)
...
def ajax_table_study(request, *args, **kwargs):
    ...
    url = 'http://127.0.0.1:8081/radiologo/rest/studies'
...
def ajax_table_series(request, *args, **kwargs):
    ...
    url = 'http://127.0.0.1:8081/radiologo/rest/studies/' + request.GET.get('studyuid') + '/series'
...
def add_item_table_study(item, session_rest, user_rest, institution_rest):
    ...
    row.append('<a href=# onClick="series(\'' + value_of('0020000D', item) + '\',\'' + pn_of('00100010', item) +
               '\',\'' + value_of('00081030', item) + '\')"><img title="Ver series" border=0 src=/static/radiologo' +
    ...
    if 'DOC' in value_of('00080061', item):
        ...
        row.append('<a href=# onClick="view_xml(\'' + uid_report['studyUID'] + '\',\'' + uid_report['seriesUID'] +
                   '\',\'' + uid_report['objectUID'] + '\')"><img title="Ver informe" border=0 src=/static/radiologo/' +
                   'img/ver_cda.png></a>')
    else:
        row.append('<a href=# onClick="report(\'' + value_of('0020000D', item) + '\')"><img title="Informe" border=0 ' +
                   'src=/static/radiologo/img/informe.png></a>')
    # download or weasis
    row.append('<a href=/weasis-pacs-connector/viewer?studyUID=' + value_of('0020000D', item) + '><img ' + \
               'title="Descargar estudio" border=0 src=/static/radiologo/img/ver_imagenes.png></a>')
    # patient id
...
def add_item_table_series(item, session_rest, user_rest, institution_rest):
    ...
    if 'OT' in value_of('00080060', item):
        uid_report = get_uid_series(value_of('0020000D', item),
                                    ...
                                    'solicitud')
        row.append('<a href=# onClick="view_xml(\'' + uid_report['studyUID'] + '\',\'' + uid_report['seriesUID'] +
                   '\',\'' + uid_report['objectUID'] + '\')"><img title="Ver solicitud" border=0 src=/static/radiologo/' +
                   'img/ver_solicitud.png></a>')
    else:
        row.append('')
    # weasis
    # row.append('<a href=/weasis-pacs-connector/viewer?studyUID=' + value_of('0020000D', item) + '&seriesUID=' +
    #           value_of('0020000E', item) + '><img title="Descargar serie" border=0 src=/static/radiologo/img/' +
    #           'ver_imagenes.png></a>')
    return row
...
def get_uid_series(study_uid, session_rest, user_rest, institution_rest, series_description):
    ...
    url = 'http://127.0.0.1:8081/radiologo/rest/studies/' + study_uid + '/series_description/' + series_description
