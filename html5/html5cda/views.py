...
def authenticate_report(submit, user):
    ...
    if institution.logo_data:
        xml_suffix += '<img height="80" width="250" src="{}"> '.format(institution.logo_data.decode('utf-8'))
        xml_suffix += '</img> '
    ...
    # qido
    url = settings.OID_URL[institution.oid]['qido'] + '/studies?StudyInstanceUID=' + values_submit['StudyIUID'][0]
    requests.get(url)
    # Active user patient
